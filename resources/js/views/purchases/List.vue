  ...
  methods: {
    ...
    handleDelete(id, name) {
      this.$confirm('This will permanently delete user ' + name + '. Continue?', 'Warning', {
        ...
        purchaseResource.destroy(id).then(response => {
          this.$message({
            type: 'success',
    ...
    async handleEditPermissions(id) {
      ...
      const { data } = await purchaseResource.permissions(id);
      this.currentUser = {
        id: found.id,
        name: found.name,
        permissions: data,
    ...
    confirmPermission() {
      ...
      purchaseResource.updatePermission(this.currentUserId, { permissions: checkedPermissions }).then(response => {
        this.$message({
